...
public class CountryContinentProcessor extends AbstractDataProcessor {
	...
	/**
	 * 
	 * @param countryISOLetterCodeName
	 *            name of the field containing the ISO 3166-1 alpha-2 country code
	 * @param continentName
	 *            name of the field where the continent standard name will be stored
	 */
	public CountryContinentProcessor(String countryISOLetterCodeName, String continentName) {
		this(countryISOLetterCodeName, continentName, CountryContinentProcessor.class.getResourceAsStream(DEFAULT_CONTINENT_COUNTRY_FILE),
				ErrorHandlingModeEnum.USE_NULL);
	}
	 ...
	 *            name of the field where the continent standard name will be stored
	 * @param input
	 *            InputStream of the Country->Continent dictionary
	 * @param errorHandlingMode
	 *            USE_NULL or USE_EMPTY
	 */
	 ...
	 * 
	 * @param in
	 *            Java bean containing the ISO country letter code as String
	 * @param out
	 *            Java bean containing the continent name as String
	 * @param params
	...
	public Continent process(String countryISOLetterCode, ProcessingResult result) {
		...
		if (continentCode != null) {
			...
			if (result != null) {
				result.addError(MessageFormat.format(resourceBundle.getString("countryContinent.error.notFound"), countryISOLetterCode));
			}
		}
		return null;
	}
